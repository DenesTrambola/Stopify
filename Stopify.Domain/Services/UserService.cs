...
public class UserService : IUserService
{
    ...
    public async Task CreateAsync(User entity)
    {
        ...
        _emailService.VerifyCode(entity.Email, "Stopify Account Activation", $"Hey there!\nHere's your Stopify Account Activation Code:\n\n{activationCode}");
    ...
    public async Task UpdateEmailAsync(string username, [StringLength(254, ErrorMessage = "Maximum length is 254!")] string newEmail)
    {
        ...
        _emailService.VerifyCode(user.Email, "Stopify Account Activation", "Hey there!\nHere's your Stopify Account Activation Code:\n\n3412");
    ...
    public async Task AddFollowerAsync(string username, string followerUsername)
    {
        ...
        if (follower is null)
            throw new EntityNotFoundException(nameof(follower));
        ...
        if (user.Followers.Any(f => f.UserId == user.Id && f.FollowerId == follower.Id))
            throw new EntityAlreadyExistsException(nameof(userFollowerItem));
    ...
    public async Task AddFollowingAsync(string username, string followingUsername)
    {
        ...
        if (followingUser is null)
            throw new EntityNotFoundException(nameof(followingUser));
